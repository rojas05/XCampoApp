...
import { View, ActivityIndicator, StyleSheet, ScrollView, Alert, Text } from "react-native";
...
const TypeUser = () => {
    ...
    async function setRol(rol, data) {
        ...
        try {
          const response = await fetchWithToken('http://192.168.0.121:8080/XCampo/api/v1/rol/'+idUser, {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            ...
            setRolData(rol.toLowerCase(),JSON.stringify(dataRequest.roles_id),data)
          }else{
      ...
      async function setRolData(rol,idRoles,data) {
        ...
        try {
          const response = await fetchWithToken('http://192.168.0.121:8080/XCampo/api/v1/'+rol+"/"+idRoles, {
            method: 'POST',
            headers: {
                    ...
                    {deliveryMan ? (     
                        <View style={styles.item}>
                            ...
                            {loading ? (
                                    ...
                                    <StyledButton 
                                    title={string.App.next} 
                                    onPress={()=>{validateForm("DELIVERYMAN")}}>
                                    </StyledButton>
                                )}
